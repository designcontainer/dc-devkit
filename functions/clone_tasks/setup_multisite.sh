...
echo -e "${error}
A Virtual hosts domain is already set for this domain. 
Please enter another local domain:
${end}"
read -e mudomain
check_if_vhosts_exist
fi
...
# Create a new array and sort if after amount of dots per domain
...
NL=$'\\\n'
target="<?php"
ms_defines="<?php${NL}${NL}define( 'MULTISITE', true );${NL}define( 'SUBDOMAIN_INSTALL', true );${NL}define( 'DOMAIN_CURRENT_SITE', '$mudomain' );${NL}define( 'PATH_CURRENT_SITE', '\/' );${NL}define( 'SITE_ID_CURRENT_SITE', 1 );${NL}define( 'BLOG_ID_CURRENT_SITE', 1 );${NL}define( 'COOKIE_DOMAIN', \$_SERVER['HTTP_HOST'] );${NL}"
